...
try {
    document.getElementById("show_passwords").addEventListener("click", () => {
        document
            .querySelectorAll(".password_input")
            .forEach((password_input) => {
            ...
            for (let i = 0; i < event.target.files.length; i++) {
                ...
                if (selected_profile_picture.type.startsWith("image/")) {
                    ...
                    read_file.addEventListener("load", () => {
                        document
                            .getElementById("product_picture_preview_div")
                            .insertAdjacentHTML(
                                "beforeend",
...
document.addEventListener("click", (event) => {
    ...
    if (
        clicked_element.id !== "search_results_div" &&
        clicked_element.id !== "search_input" &&
        clicked_element.id !== "search_btn"
    )
...
function switch_tab(idx = -1) {
    ...
    for (let i = 0; i < all_tabs.length; i++) {
        if (i != idx) all_tabs[i].style.display = "none";
        else all_tabs[i].style.display = "flex";
    }
}
...
function next_picture(particular_product) {
    // change style of the tab title to potray current tab
    const particular_each_pic_div = particular_product.parentElement
        .querySelector(".product_pictures_slideshow_div")
    ...
    for (let i = 0; i < particular_each_pic_div.length; i++) {
        if (particular_each_pic_div[i].classList.contains("active")) {
            ...
            else particular_each_pic_div[i + 1].classList.add("active");
            break;
        }
    }
}
...
function prev_picture(particular_product) {
    ...
    for (let i = 0; i < particular_each_pic_div.length; i++) {
        if (particular_each_pic_div[i].classList.contains("active")) {
            ...
            if (i == 0)
                particular_each_pic_div[
                    particular_each_pic_div.length - 1
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        flash_btn = document.getElementById(
            "display_flashed_messages_modal_btn"
        );
        flash_btn.click();
        flash_btn.remove();
    } catch {
    ...
    try {
        ...
        if (
            document.getElementById("particular_product_description_h5")
                .textContent.length > 800
        )
            document.getElementById(
                "product_information_and_quantity_form_div"
        ...
        // check if the product name is more than 35 characters and the product name is more than 400 characters and make the div scrollable
            ...
            document.getElementById("store_name_div").children[0].textContent
                .length > 30 &&
            document.getElementById("particular_product_description_h5")
...
function display_search_results(results) {
    ...
    search_results_div.style.zIndex = "1000000";

    for (let i = 0; i < results.length; i++) {
        search_results_div.insertAdjacentHTML(
            "beforeend",
            `
...
function increaseSearchResultsDivMarginTop() {
    ...
    if (window.innerWidth <= 990)
        document
            .getElementById("search_results_div")
            .classList.add("more-margin");
}
